...
bool    RobotomyRequestForm::execute(const Bureaucrat& executor) const
{
	...
	{
		std::cout << " * PROCEDURE OVER * WE APPRECIATE YOU TRUSTING US FOR YOUR ROBOTOMY NEEDS * blip boop OUYYYUOOUYYUOOUYYUOO !!! chzk * Diagnostic : Low resistance to frontal cortex trimming * " << std::endl;
		std::cout << " * " << _target << " * PRESENT YOURSELF TO YOUR LOCAL BIOHAZARD FACILITY FOR DISPOSAL * GOOD BYE * " << std::endl;
	}
	return (true);
}
